...
async function loadAsegConfig(slug) {
  ...
  // formato de fecha: config → env → default
  const formato =
...
async function cotizarFila({
  fila,
  cabecera,
  hoy_ddmmyyyy, // ahora puede venir en distintos formatos según config
  mapeos,
  Aseg,
  SOAP_URL,
  SOAP_METHOD,
  ...
  // Defaults vehículo desde cabecera (parámetros típicos de cotización)
...
router.post('/ejecutar/:id', express.json(), async (req, res) => {
  try {
    ...
    if (!cabecera)
      return res
        .status(404)
        .json({ ok: false, error: `Cabecera ${cabeceraId} no encontrada` });
    ...
    if (!rows || rows.length === 0)
      return res.status(404).json({ ok: false, error: `No existe histórico id=${id}` });
    const item = rows[0];
    const relPath =
      item.ruta && item.ruta.trim()
        ? item.ruta
    ...
    if (!fs.existsSync(absPath))
      return res
        .status(400)
        .json({ ok: false, error: `No se encuentra el archivo combinado: ${absPath}` });
    ...
    if (absPath.toLowerCase().endsWith('.csv')) {
      ...
      const hoja =
        wb.SheetNames.find(
          (n) => xlsx.utils.sheet_to_json(wb.Sheets[n], { defval: '' }).length > 0
        ) || wb.SheetNames[0];
    ...
    for (let i = 0; i < tomar; i++) {
      ...
      resultados.push({
        ...
        fila_preview: {
          infoautocod:
            fila_preparada.infoautocod ??
            fila.infoautocod ??
            fila.tau_codia ??
            '',
          anio:
            fila_preparada.anio ||
            fila_preparada.anofab ||
            fila.anio ||
            fila.anofab ||
            '',
          cp:
            fila_preparada.cp ||
            fila_preparada.codigo_postal ||
            fila.codigo_postal ||
            fila.cp ||
            '',
      ...
      try {
        fs.writeFileSync(
          path.join(procDir, 'last_soap_response.xml'),
          String(resp.raw || ''),
          'utf8'
        );
      } catch {}
    ...
    fs.writeFileSync(
      path.join(procDir, 'resumen.json'),
