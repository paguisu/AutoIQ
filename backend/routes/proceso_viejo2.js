...
// ===== Aseguradoras disponibles (desde /data/<slug>/aseguradora.json) =====
async function listAseguradorasDisponibles() {
  const dataDir = path.join(process.cwd(), 'data');
  let ents = [];
  try {
    ents = await fsp.readdir(dataDir, { withFileTypes: true });
  } catch {
    return [];
  }
  ...
  for (const ent of ents) {
    ...
    const slug = ent.name;
    const cfgPath = path.join(dataDir, slug, 'aseguradora.json');
    ...
    try {
      ...
      out.push({ slug, nombre_publico: slug, activo: true });
    }
  }

  out.sort((a, b) => String(a.nombre_publico).localeCompare(String(b.nombre_publico)));
  return out;
}
...
router.get('/aseguradoras-disponibles', async (_req, res) => {
  try {
    const items = await listAseguradorasDisponibles();
...
router.post('/crear', express.json(), async (req, res) => {
  try {
    ...
    let aseguradoras = req.body?.aseguradoras ?? req.body?.aseguradoras_slugs ?? req.body?.aseguradoras_ids;
    if (typeof aseguradoras === 'string') aseguradoras = [aseguradoras];
    if (!Array.isArray(aseguradoras)) aseguradoras = [];
    // Normalizar: solo slugs (strings). Si llegan números, se ignoran.
